...
const DataManager = {
    ...
    checkAchievements() {
        ...
        const totalDistance = App.data.sessions.reduce((sum, s) => sum + s.distance, 0);
        const completedTasks = App.data.tasks.filter(t => t.completed).length;
        
        // Проверка и разблокировка достижений
        const achievements = Utils.load('achievements', {
            firstSteps: false,      // 1000 шагов
            marathon: false,          // 42 км -> 3 задачи
            weekStreak: false,      // 7 дней
            master: false           // 100000 шагов
        });
        
        // Первые шаги
        if (totalSteps >= 1000) achievements.firstSteps = true;
        
        // Марафонец -> теперь 3 задачи
...
function toggleSound() {
    ...
    Utils.save('settings', App.data.settings);
    event.target.textContent = App.data.settings.sound ? '🔊' : '🔇';
}
...
function saveTask(event) {
    ...
    renderTasks();
    updateTaskStats();
    updateDashboardTasks(); // Обновляем на главной
}
...
function renderAchievements() {
    ...
    // Обновляем достижения в реальном времени
    ...
    cards.forEach(card => {
        ...
        if (name === 'Марафонец' && completedTasks >= 3) unlocked = true; // Теперь 3 задачи!
...
function initProfilePage() {
    ...
    // Обработчик темы с переключением
    if (darkModeEl) {
...
function changeAvatar() {
    showNotification('Функция загрузки фото в разработке');
}
    ...
    input.onchange = (e) => {
        ...
        reader.onload = (event) => {
            try {
                ...
                if (data.user) App.data.user = data.user;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Добавление стилей для анимаций и утилит
    ...
    style.textContent = `
        ...
        /* Фикс для белого текста на белом фоне */
    ...
    // SVG Gradient для кольца прогресса
